...
import { ArrowLeft, MapPin, Plus, Store, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-card">
      <div className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link href="/admin" className="flex items-center gap-2 hover:opacity-75">
            <ArrowLeft className="w-5 h-5" />
            <span className="font-semibold hidden sm:inline">Admin</span>
          </Link>
          <h1 className="font-bold">Boutiques</h1>
          <button
            type="button"
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            {stores.map((store) => (
              <article key={store._id} className="bg-card border border-border rounded-xl p-5">
                ...
                <div className="rounded-lg bg-muted/50 p-3 text-sm">
                  ...
                  <p className="text-muted-foreground">{store.managerId?.email}</p>
                </div>
                <span
        ...
        {stores.length === 0 && !loading && (
          <p className="text-center text-muted-foreground py-16">Aucune boutique. Créez la première.</p>
        )}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          >
            ...
            <button type="submit" className="w-full mt-4 py-3 bg-primary text-primary-foreground rounded-lg font-semibold">
              Créer la boutique
            </button>
          </form>
        </div>
